...
public class CheckProductDetailsAllOverPagesTest extends TestBase {
    ...
    public void getProductDetails() {
        ...
        WebElement product = home.getRandomProductContainer();
        waitFor().until(ExpectedConditions.elementToBeClickable(product));
        productTitle = home.getHomeProductTitle(product);
        productPrice = home.getHomeProductPrice(product);
    ...
    public void checkProductPageDetails() {
        ...
        String productPageProductTitle = product.getProductTitle();
        String productPageProductPrice = product.getPrice();
        Assert.assertEquals(productPageProductTitle, productTitle);
        Assert.assertEquals(productPageProductPrice, productPrice);
    ...
    @Test(dependsOnMethods = {"checkProductPageDetails"})
    public void checkCartPageDetails() {

        cart = new Cart(driver);
        Cart.CartProduct product = cart.getProduct(1);

        String cartPageProductTitle = product.getProductTitle();
        String cartPageProductPrice = product.getUnitPrice();
        int cartPageProductQuantity = product.getQuantity();
        Assert.assertEquals(cartPageProductTitle, productTitle);
        Assert.assertEquals(cartPageProductPrice, productPrice);
        Assert.assertEquals(cartPageProductQuantity, productQuantity);
    ...
    @Test(dependsOnMethods = {"checkCartPageDetails"})
    public void checkCheckoutPageDetails() {

        checkout = new Checkout(driver);
        Checkout.CheckoutProduct product = checkout.getProduct(1);

        String checkoutPageProductTitle = product.getProductTitle();
        String checkoutPageProductPrice = product.getUnitPrice();
        int checkoutPageProductQuantity = product.getQuantity();
        Assert.assertEquals(checkoutPageProductTitle, productTitle);
        Assert.assertEquals(checkoutPageProductPrice, productPrice);
        Assert.assertEquals(checkoutPageProductQuantity, productQuantity);
